...
**Live URL**: https://lunchbox.shinyapps.io/mbb_radar_app/  
...
### `manifest.json`
Generated by `rsconnect::writeManifest()`. Lists all files and package versions for shinyapps.io deployment. Regenerate with `rsconnect::writeManifest()` before deploying if you add files.

### `rsconnect/`
Metadata folder created by the rsconnect package. Contains the deployment configuration for shinyapps.io. Not needed for local development.
...
The app is deployed to shinyapps.io under the account `lunchbox`.
...
The `SUPABASE_DB_PASSWORD` environment variable must be set in the shinyapps.io app settings (Posit Connect dashboard → App → Environment Variables). It is not deployed from the local `.Renviron`.
